...
void ompl::control::RGRRT::setupReachableSet(Motion* const m)
{
    ...
    for (int i = 0; i < this->RSIZE; i++)
    {
        ...
        {
            controls[j] = low_bound[j] + control_offset_[j] * (i + 1);
            for (int k = 1; k < low_bound.size(); k++)
                controls[k] = low_bound[0];
        }
        
        
        // DEBUGGING
        std::cout << "Initial state before propagation: ";
        si_->printState(m->state, std::cout);
        std::cout << std::endl;

        motion->steps = siC_->propagateWhileValid(m->state, motion->control, siC_->getMinControlDuration(), motion->state);
        std::cout << "Steps after propagation: " << motion->steps << std::endl;
        // siC_->propagate(m->state, motion->control, siC_->getMinControlDuration(), motion->state);

        
        if (motion->steps == 0) {
            std::cerr << "Propagation failed. State remains invalid." << std::endl;
            si_->freeState(motion->state);
            siC_->freeControl(motion->control);
            delete motion;
            continue;
        ...
        si_->printState(motion->state, std::cout);
        std::cout << std::endl;

        m->reachable.push_back(motion);
        std::cout << "Added motion to reachable set with state: ";
        si_->printState(motion->state, std::cout);
        std::cout << std::endl;
...
int ompl::control::RGRRT::selectReachableMotion(const Motion* qnear, const Motion* qrand)
{
    ...
    // std::cout << "Distance between qnear and qrand: " << near_distance << std::endl;
    ...
    for (int i = 0; i < reachable.size(); i++)
    {
        ...
        // std::cout << "Distance between reachable and qrand: " << near_distance << std::endl;
...
ompl::base::PlannerStatus ompl::control::RGRRT::solve(const base::PlannerTerminationCondition &ptc)
{
    ...
    while (!ptc)
    {
        ...
        while (id == -1){
            ...
            // if (id != -1) {
            //     std::cout << "Selected reachable motion's state: ";
            //     si_->printState(nmotion->reachable[id]->state, std::cout);
            //     std::cout << std::endl;
            // }
            // else {
            //     std::cout << "No reachable motion found." << std::endl;
            // }
        }
